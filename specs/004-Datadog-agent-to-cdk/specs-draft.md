...
- Related specs: `../003-OTel-to-backend/specs-draft.md`（backend 側）
...
- Datadog 連携に必要な組織情報
  ...
  - Datadog Organization 名（または対象アカウント識別子）
  - Datadog API Key の保管先（Secrets Manager 名/ARN、ローテーション責任者）
  - Datadog Application Key 要否（monitor/dashboard/index を API/IaC 管理する場合は必要）
  - Application Key を使う場合の最小権限スコープ
