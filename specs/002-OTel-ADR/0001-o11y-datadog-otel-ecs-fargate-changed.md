...
- Supersedes: N/A
- Superseded by: N/A
- Related specs: `specs-draft.md`（作成予定）
...
Spring Boot では、Starter を利用する場合、デフォルトのログ実装として Logback が使用される。したがって、現在の `SLF4J + Logback` 構成は Spring Boot アプリケーションとして一般的であり、アプリケーションログを出力する仕組みとしては継続利用する。CloudWatch Logs 中心の運用から、Datadog を主要画面とする運用へ移行する。
...
```mermaid
flowchart TB
  DDAG["Datadog Agent sidecar"] --> DDAG_ROLE["traces / metrics / APM / OTLP の受け口"]
  FLB["FireLens / Fluent Bit"] --> FLB_ROLE["stdout / stderr のアプリケーションログを Datadog Logs へ送るログルーター"]
```

## 2. 課題

CloudWatch Logs だけでは、次のような調査が難しい。
...
- 調査画面が CloudWatch Logs と Datadog に分散し、運用手順が揺れること

## 3. 決定ドライバー

この意思決定では、次を重視する。

- 運用の分かりやすさ: 障害調査の主画面を Datadog に統一できること
- 相関性: logs / traces / metrics を同一の `service` / `env` / `version` で横断できること
- コスト: ログ二重保存や高カーディナリティ属性による課金増を抑制できること
- セキュリティ / コンプライアンス: PII / secret を出力しない方針を実装・運用に落とせること
- 既存影響: 既存の `SLF4J + Logback` を維持し、業務コード改修を最小化できること
...
注意:
- ECS の `logDriver` はコンテナ単位で 1 つのみ設定できるため、同一コンテナで `awslogs` と `awsfirelens` を同時設定することはできない。
- 同一タスク内ではコンテナごとに別ドライバを設定できるため、application container は `awsfirelens`、log_router / datadog-agent container は `awslogs` のような併用は可能である。
- application container のログを Datadog と CloudWatch の両方に出したい場合は、`awsfirelens` を使い、FireLens（Fluent Bit）の複数出力で転送する。

#### 4.1.2 アプリケーションログは SLF4J + Logback を継続利用する
...
#### 4.1.3 アプリケーションログは stdout に JSON 形式で出力する
...
#### 4.1.4 アプリケーションログは FireLens / Fluent Bit で Datadog Logs へ送信する

アプリケーションコンテナのログドライバーは、最終構成では `awsfirelens` を使用する。

```mermaid
flowchart LR
  APP["Spring Boot app"] -- "stdout / stderr" --> FL["FireLens / Fluent Bit"]
...
#### 4.1.8 metrics は集計目的に限定し、高カーディナリティ属性を避ける
...
#### 4.1.9 Datadog Unified Service Tagging を標準化する
...
- FireLens / Fluent Bit を traces / metrics 転送基盤として使う構成
- Datadog Agent sidecar なしで observability を完結させる構成
- OpenTelemetry Logs API を通常ログの主要 API として使う構成

### 4.3 例外

- 監査要件により CloudWatch Logs にログ保持が必要な場合のみ、保持期間を明示したうえで併用を認める。
- 例外を追加・拡大する場合は、本 ADR を更新して承認履歴を残す。
...
flowchart TB
  subgraph TASK["ECS Fargate Task"]
    APP["Spring Boot App Container<br/>- SLF4J + Logback<br/>- JSON logs to stdout<br/>- OpenTelemetry Spring Boot Starter<br/>- Manual instrumentation (@WithSpan / OTel API)<br/>- OTLP traces / metrics exporter"]
    FL["FireLens / Fluent Bit Container<br/>- awsfirelens log router<br/>- Datadog Fluent Bit output plugin"]
    AG["Datadog Agent Container<br/>- ECS_FARGATE=true<br/>- APM enabled<br/>- OTLP receiver enabled"]
  end
  ...
  AG -. "診断ログ（必要時のみ）" .-> CW
  CW --- CWM["用途: 運用上の保険 / 監査要件 / 短期保持"]
```

## 6. 検討した代替案

### 6.1 CloudWatch Logs と Datadog Logs の両方でアプリログを全量・長期保持する
...
### 6.2 通常ログは CloudWatch Logs、OpenTelemetry logs は Datadog Logs で見る
...
### 6.3 FireLens のみを使用し、Datadog Agent sidecar を使用しない
...
### 6.4 Datadog Agent sidecar のみを使用し、FireLens を使用しない
...
### 6.5 OpenTelemetry Collector sidecar を使用し、Datadog Agent sidecar を使用しない
...
### 6.6 OpenTelemetry Logs API をアプリケーションの業務ログ API として直接使用する
...
- OTLP エクスポーターは HTTP 固定とし、`OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf`、`OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318` を設定する。
...
### 8.2 ECS Fargate タスク定義

#### 8.2.1 コンテナ構成

最終構成では、次のコンテナを同一タスク定義に含める。

1. application container
2. Datadog Agent container
3. FireLens / Fluent Bit log router container

#### 8.2.2 application container
...
#### 8.2.4 FireLens / Fluent Bit container
...
- `compress=gzip` を設定する。
- Log router のメモリ不足によるログロストを避けるため、タスク CPU / memory に log router 分を上乗せする。
...
- エラー追跡: `service:todo-backend env:prod status:error`
...
CloudWatch Logs を残す場合の保持期間を次で固定する。
...
この ADR の決定が実装されたと判断する条件は次の通りである。

- [ ] Spring Boot アプリが `SLF4J + Logback` で JSON ログを stdout に出力している。  
確認手順: `backend/` で `./mvnw -DskipTests compile` を実行し、`src/main/resources` の Logback 設定で JSON appender と Console 出力を確認する。
- [ ] アプリログに `service` / `env` / `version` / `trace_id` / `span_id` が含まれている。  
確認手順: Datadog Logs Explorer で `service:todo-backend env:prod @trace_id:* @span_id:*` を実行し、対象フィールドが抽出されることを確認する。
- [ ] アプリログが FireLens / Fluent Bit 経由で Datadog Logs に到達している。  
確認手順: ECS タスク定義で application container の `logDriver=awsfirelens` を確認し、Datadog Logs に新規ログが到達することを確認する。
- [ ] CloudWatch Logs がアプリログの主要閲覧先ではなくなっている、または保持期間・用途が明示されている。  
確認手順: ECS タスク定義の log configuration と CloudWatch Log Group retention を確認し、本 ADR の 10.3 と一致することを確認する。
...
- [ ] Datadog 上で logs / traces / metrics が同じ `env` / `service` / `version` で紐づく。  
確認手順: 各画面で `service:todo-backend env:prod version:<container-image-tag>` フィルタを適用して同一リリースのデータが表示されることを確認する。
- [ ] Datadog Logs の Index / Exclusion Filter / retention / quota の方針が定義されている。  
確認手順: Datadog Logs Configuration 画面で対象設定が作成済みであることを確認する。
- [ ] PII / secret を出さないための実装・レビュー観点が明文化されている。  
確認手順: 本 ADR の 11 章と、開発時レビュー観点に同内容が反映されていることを確認する。
...
- ロールバック時は、次の順序で戻す。
  ...
  2. Datadog Logs 向け FireLens 出力を停止する。
  3. Datadog Agent OTLP receiver を停止する。
  4. CloudWatch Logs の保持期間を運用標準へ戻す。
- 本方針を恒久的に変更する場合は、新しい ADR を作成して本 ADR を `Superseded` にする。
...
- Datadog Trace Explorer Query Syntax: https://docs.datadoghq.com/ja/tracing/trace_explorer/query_syntax/
- Datadog Log Search Syntax: https://docs.datadoghq.com/ja/logs/search-syntax
