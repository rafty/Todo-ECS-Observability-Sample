...
flowchart TB
  subgraph TASK["ECS Fargate Task"]
    APP["Spring Boot App Container<br/>- SLF4J + Logback<br/>- JSON logs to stdout<br/>- OpenTelemetry Spring Boot Starter<br/>- @WithSpan / OpenTelemetry API manual instrumentation<br/>- OTLP traces / metrics exporter"]
    FL["FireLens / Fluent Bit Log Router<br/>- awsfirelens<br/>- Datadog Fluent Bit output"]
    AG["Datadog Agent Sidecar<br/>- ECS_FARGATE=true<br/>- APM enabled<br/>- OTLP receiver enabled"]
  end
...
注意:
- 同一コンテナでは `logDriver` を 1 つしか設定できないため、`awslogs` と `awsfirelens` の同時設定はできない。
- 同一タスク内ではコンテナごとに別ドライバを設定できるため、application container を `awsfirelens`、log_router / datadog-agent container を `awslogs` にする併用は可能である。
...
本仕様では、OTLP/HTTP を固定で採用する。Datadog Agent に次を設定する。
...
`backend/` の現行構成では Logback JSON フィールドの独自名称は定義されていないため、`dd_message_key` は Logback JSON の標準的な `message` を使用する。
...
CloudWatch Logs を残す場合の保持期間は次で固定する。
...
OTEL_RESOURCE_ATTRIBUTES=service.name=todo-backend,service.version=<version>,deployment.environment=prod
```
...
- ECS タスク定義確認:
  - app container が `logDriver=awsfirelens` であること
  - app container が `OTEL_EXPORTER_OTLP_PROTOCOL=http/protobuf` と `OTEL_EXPORTER_OTLP_ENDPOINT=http://localhost:4318` を持つこと
  - log_router / datadog-agent の CloudWatch Logs retention が `dev=3日, stg=7日, prod=14日` であること
...
- OTLP/HTTP 経由で Datadog APM / Metrics に telemetry が届くことを確認する。
