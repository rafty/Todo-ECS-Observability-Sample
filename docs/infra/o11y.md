...
- `Trace / Span（手動計装）` は Java Agent が自動生成しない Todo 業務操作に限定し、OpenTelemetry API で作成する。
...
最初に理解すべき点は、`logs / traces / metrics` は同じリクエストから作られることがあるが、同じデータではなく、同じ経路でもないという点である。

| Signal | 何を見るものか | この構成での作り方 | なぜ別経路にするか |
| --- | --- | --- | --- |
| logs | 「何が起きたか」を読む。業務イベント、監査証跡、例外など | アプリが SLF4J で JSON を stdout へ出す | ログ配送は FireLens が得意で、ECS メタデータ付与や Datadog Logs 送信を JVM から分離できるため |
| traces / spans | 「どこを通り、どこで時間がかかったか」を見る | OpenTelemetry Java Agent が HTTP / Spring / JDBC span を作り、アプリが Todo 業務 span を補う | アプリ処理の親子関係と所要時間を APM で見るため |
| metrics | 「どれくらい起きたか」「傾向が変わったか」を数値で見る | Java Agent が Runtime / JDBC / Micrometer metrics を OTLP で送る | 集計・アラート・ダッシュボードで使うため |

`event` という言葉は混同しやすい。この文書で「業務イベント」「ログイベント」と書く場合は Datadog Logs に入る JSON ログのことを指す。OpenTelemetry の `span event` は span にぶら下がる補助情報であり、このプロジェクトでは監査証跡や業務履歴の主経路にはしない。

```mermaid
sequenceDiagram
  participant Client as Browser
  participant App as TodoBackendContainer
  participant FireLens as LogRouterContainer
  participant Agent as DatadogAgentContainer
  participant Logs as Datadog Logs
  participant APM as Datadog APM
  participant Metrics as Datadog Metrics

  Client->>App: /api/todos
  App->>App: JSONログをstdoutへ出力
  App->>App: HTTP / Spring / JDBC spanを生成
  App->>App: todo.operation.* metricsを記録
  App->>FireLens: stdout / stderr logs
  FireLens->>Logs: Datadog Logs plugin
  App->>Agent: OTLP traces gRPC 4317
  App->>Agent: OTLP metrics HTTP 4318
  Agent->>APM: traces / spans
...
この章は、[backend のログ / 手動業務テレメトリ設計](../backend/logging.md) と同じ分類を、ECS task 内の配送経路が分かる形で整理する。backend 文書は「どのコードが何を作るか」を説明し、この文書は「作られた logs / trace / metrics がどの sidecar を通って Datadog に届くか」を説明する。

この構成では、ログ出力、自動計装、手動計装を混ぜて扱わない。

- ログ出力は、アプリが SLF4J で出した JSON ログを FireLens が Datadog Logs へ送る経路である。
- 自動計装は、OpenTelemetry Java Agent が HTTP / Spring / JDBC / Runtime などから trace/span や metrics を作る経路である。
- 手動計装は、アプリコードが Todo 業務単位の span や metrics を明示的に追加する経路である。送信は Java Agent と Datadog Agent の OTLP 経路に乗せる。

| 対象 | 自動計装/手動計装/ログ出力 | 生成・実装 | ECS task 内の経路 | Datadog 側 | 補足 |
| --- | --- | --- | --- | --- | --- |
| アプリログ | ログ出力 | Spring Boot structured logging、SLF4J、MDC、`logging` パッケージ | app stdout / stderr -> FireLens (`awsfirelens`) | Datadog Logs | app コンテナは CloudWatch Logs へ直接送らない。`OTEL_LOGS_EXPORTER=none` とし、OTLP logs 経路も使わない |
| HTTP request trace / framework span | 自動計装 | `opentelemetry-javaagent.jar` を `JAVA_TOOL_OPTIONS` で JVM に attach する | app -> OTLP/gRPC `localhost:4317` -> Datadog Agent | Datadog APM | Java Agent が HTTP server span を起点に trace を作り、Servlet / Spring Web MVC / JDBC span を関連付ける |
| Todo 業務 span | 手動計装 | `TodoOperationTelemetryAspect`、`TodoOperationSpanService` が OpenTelemetry API と `GlobalOpenTelemetry` を使う | app -> OTLP/gRPC `localhost:4317` -> Datadog Agent | Datadog APM | 通常は Java Agent が作った HTTP request trace の一部として追加される。対象は `todo.list|get|create|update|delete` に限定する |
| JDBC / Runtime metrics | 自動計装 | OpenTelemetry Java Agent の JDBC / Runtime instrumentation | app -> OTLP/HTTP `localhost:4318/v1/metrics` -> Datadog Agent | Datadog Metrics | JDBC / HikariCP / JVM Runtime metrics を Java Agent 側で取得する |
| Todo 業務 metrics | 手動計装 | `BusinessMetricsService` が Micrometer `MeterRegistry` に `todo.operation.*` を記録し、Java Agent Micrometer instrumentation が export する | app -> OTLP/HTTP `localhost:4318/v1/metrics` -> Datadog Agent | Datadog Metrics | アプリコードは Micrometer API を維持する。Datadog Metrics API へ直接送らない |
...
この構成では、同じ「Observability」でも logs / trace / metrics ごとに最適な担当を分ける。
...
Datadog UI 上の表示名は、環境・Agent バージョン・OTLP mapping に依存します。そのため、未確認の metric 名は断定しません。実環境で確認済みの metric API 名は本章の一覧を正とします。
...
| 分類 | 確認済み metric API 名 | 読み方 |
| --- | --- | --- |
| Todo 業務 metrics | `todo.operation.count` | Todo 操作の成功/失敗件数。`operation` と `result` で集計する |
| Todo 業務 metrics | `todo.operation.duration`、`todo.operation.duration.max` | Todo 操作の処理時間。`duration` は分布、`.max` は期間内最大値として扱う |
| JDBC / database client metrics | `db.client.operation.duration`、`db.client.connection.*` | DB 操作時間と DB client 接続関連の系列 |
| HikariCP / DB pool metrics | `hikaricp.connections.*` | HikariCP connection pool の使用状況 |
| JVM Runtime Metrics | `jvm.memory.*`、`jvm.gc.*`、`jvm.threads.*`、`jvm.classes.*`、`jvm.cpu.*` | Java Agent runtime telemetry 由来の JVM 状態 |
| Process / CPU metrics | `process.cpu.*`、`system.cpu.*` | アプリプロセスや実行環境の CPU 関連系列 |

`*` は prefix 配下の複数 metric をまとめて表す。個別名は Java Agent、Datadog Agent、Datadog 側 OTLP mapping のバージョンで増減する可能性があるため、運用手順や dashboard では Metrics Explorer / API で実在名を確認してから固定する。
